...
func (s *URLTest) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	...
	if err == nil {
		// lx-port SPEC-019 passive_check — a successful TCP dial proves two-way
	...
	// lx-port SPEC-019 v2 — in round_robin a dial error must NOT touch the pool: the cause is
...
type URLTestGroup struct {
	...
	lastSelected common.TypedValue[string] // lx-port SPEC-019 — Now() in balanced modes
	balancer     *balancer                 // lx-port SPEC-019 v2 — round_robin pool; nil for least_test
	groupTag     string                    // lx-port SPEC-019 — group's own tag (used by pool APIs)
	// lx-port SPEC-019 passive_check — tag → unix-nano of the last successful TCP
...
func (g *URLTestGroup) loopCheck(ticker *time.Ticker, closeChan <-chan struct{}) {
	...
	for {
		...
		// (lx-port omits SPEC-020 reachability gate — no ReachabilityReporter here.)
		g.CheckOutbounds(false)
	}
}
...
func (g *URLTestGroup) urlTest(ctx context.Context, force bool) (map[string]uint16, error) {
	...
	// lx-port SPEC-019 passive_check (least_test) — while the currently selected
	// node is passively confirmed alive (fresh successful TCP dial through it),
	// skip the whole periodic re-test cycle: nothing is broken, so don't wake N-1
